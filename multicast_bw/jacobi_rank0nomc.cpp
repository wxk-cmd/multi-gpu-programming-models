// Test: rank0 (per-node local_rank==0) sends via Multicast VA,
// receivers (local_rank!=0) bind UC physical memory only.
// Sender does NOT create/bind UC; it only maps MC VA and writes.
//
// Compile (Hopper / SM90+):
//   nvcc -O3 -std=c++17 -arch=sm_90 -c mc_multicast_only_kernels.cu -o mc_multicast_only_kernels.o
//   mpicxx -O3 -std=c++17 rank0_no_uc_mc.cpp mc_multicast_only_kernels.o \
//     -I${CUDA_HOME}/include -L${CUDA_HOME}/lib64 -lcuda -lcudart -o test_mc_rank0_no_uc
//
// Run example (per your cluster):
//   mpirun -np 8 -H nodeA,nodeA,nodeA,nodeA,nodeA,nodeA,nodeA,nodeA \
//     ./test_mc_rank0_no_uc -N 134217728 -iters 200 -warmup 20
//
// Flags:
//   -csv         : print csv line
...
// ---------- external kernel launcher (kept for parity; not used here) ----------
...
int main(int argc, char** argv) {
    ...
    // Per-node role
    const bool is_sender   = (local_rank == 0);
    
    const bool is_receiver = !is_sender;   // local_rank != 0

    // args
    const size_t N       = get_argval<size_t>(argv, argv+argc, std::string("-N"),      size_t(1)<<26); // default smaller for safety
    ...
    mc_prop.numDevices  = world;                    // all ranks participate
    ...
    // Add this device to multicast (sender and receivers all add)
    ...
    // Only receivers create UC physical memory and bind it to multicast
    CUmemGenericAllocationHandle uc_handle_slots{};
    CUdeviceptr uc_ptr_slots = 0;
    if (is_receiver) {
        ...
        // Bind this device's physical mem into multicast object at offset 0
        CUDA_CALL(cuMulticastBindMem(mc_handle_slots, 0 /*mc_off*/, uc_handle_slots, 0 /*uc_off*/, uc_size, 0));

        // Optionally: map UC to this process VA so receiver can read/verify
    ...
    // Map MC to VA on all ranks (sender needs it to write; receivers can skip reads if not needed)
    CUdeviceptr mc_ptr_slots=0;
    {
    ...
    real* uc_slots = reinterpret_cast<real*>(uc_ptr_slots); // nullptr on sender

    // ---------- build MC/UC for 4B cumulative counter (same policy) ----------
    ...
    if (is_receiver) {
        ...
        // init counter on receivers
        CUDA_RT_CALL(cudaMemset(reinterpret_cast<void*>(uc_ptr_ctr), 0, sizeof(unsigned int)));
    }
    // Map counter MC VA for all (sender may write if you add barriers)
    CUdeviceptr mc_ptr_ctr=0;
    {
    ...
    auto* counter_uc = reinterpret_cast<unsigned int*>(uc_ptr_ctr); // nullptr on sender
    ...
    for (int w=0; w<warmup; ++w) {
        if (is_sender) {
            // sender writes to slot 0 (offset 0). Hardware replicates to receivers' UC pages.
            CUDA_RT_CALL(cudaMemcpyAsync(mc_slots /* + 0*N */,
                                         d_in,
                                         N * sizeof(real),
    ...
    const double bytes_per_iter_sender = double(N) * double(sizeof(real));  // one write by sender
    const double my_bytes_per_iter     = (is_sender) ? bytes_per_iter_sender : 0.0;
    const double total_bytes_per_rank  = my_bytes_per_iter * double(iters);
    const double gb_per_rank           = total_bytes_per_rank / 1.0e9;
    const double gbps_per_rank         = secs > 0 ? (gb_per_rank / secs) : 0.0;

    // Delivered bytes (replicated to #receivers). If there are R receivers per-node, but
    // here we don't compute per-node; we count worldwide receivers as (#ranks - #senders)
    // Assuming one sender per node; to be precise, compute based on your topology as needed.
    // As a simple model: delivered_gb_per_rank = gb_per_rank * (#receivers_worldwide)
    // We approximate senders = number of nodes (unknown here). To avoid wrong math,
    // we just multiply by (world - 1) as an upper-bound if every node runs one sender.
    const int receivers_upper_bound = (world > 1 ? world - 1 : 0);
    const double delivered_gb_per_rank   = (is_sender) ? (gb_per_rank * double(receivers_upper_bound)) : 0.0;
    const double delivered_gbps_per_rank = secs > 0 ? (delivered_gb_per_rank / secs) : 0.0;

    // Reduce sender gbps to get sum/avg across world (only senders contribute)
    double sum_gbps=0.0;
    MPI_CALL(MPI_Allreduce(&gbps_per_rank, &sum_gbps, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD));
    const double avg_gbps = sum_gbps / double(world);

    if (csv) {
        if (rank==0) {
            printf("mc_multicast_rank0_no_uc,%d,%d,%zu,%d,%d,%s,%.6f,%.6f,%.6f\n",
        ...
        if (is_sender) {
            printf("[Rank %d | Dev %d | local_rank %d] N=%zu | sender write: %.3f GB in %.3f s => %.3f GB/s\n",
                   rank, dev_id, local_rank, N, gb_per_rank, secs, gbps_per_rank);
            printf("[Rank %d] delivered (replicated to ~%d peers): %.3f GB/s\n",
                   rank, receivers_upper_bound, delivered_gbps_per_rank);
        }
        if (rank==0) {
            printf("World=%d | Avg per-rank BW (incl. silent ranks): %.3f GB/s | Aggregate(active senders): %.3f GB/s | dtype=%s\n",
