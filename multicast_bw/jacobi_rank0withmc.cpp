// Test for multicast-only 1D communication (rank0-only sender).
// Build & run are same as the original file.
//
// Compile (Hopper / SM90+):
//   nvcc -O3 -std=c++17 -arch=sm_90 -c mc_multicast_only_kernels.cu -o mc_multicast_only_kernels.o
//   mpicxx -O3 -std=c++17 rank0_only_mc.cpp mc_multicast_only_kernels.o \
//     -I${CUDA_HOME}/include -L${CUDA_HOME}/lib64 -lcuda -lcudart -o test_mc_rank0
//
// Run example:
//   mpirun -np 4 ./test_mc_rank0 -N 134217728 -iters 200 -warmup 20
//
// Flags:
//   -csv         : print csv line
//   -gather      : (unused here) reserved
...
// ---------- external kernel launcher (unused path kept for completeness) ----------
...
int main(int argc, char** argv) {
    ...
    // args
    const size_t N       = get_argval<size_t>(argv, argv+argc, std::string("-N"),      size_t(1)<<30);
    ...
    { // UC physical memory on this device
    ...
    // ---------- build MC/UC for 4B cumulative counter (kept for parity; not used here) ----------
    ...
    // Optional gather buffer (unused in this rank0-only write test)
    real* d_all = nullptr;
    if (doGather) {
    ...
    for (int w=0; w<warmup; ++w) {
        if (rank == 0) {
            // rank0 写入 slot 0（偏移 0）
            CUDA_RT_CALL(cudaMemcpyAsync(mc_slots /* + 0 * N */,
                                         d_in,
                                         N * sizeof(real),
                                         cudaMemcpyDeviceToDevice,
                                         stream));
            // 可选：如果希望 rank0 把同一份数据写到所有 slot，解开下段
            // for (int s = 0; s < world; ++s) {
            //     CUDA_RT_CALL(cudaMemcpyAsync(mc_slots + (size_t)s * N,
            //                                  d_in, N * sizeof(real),
    ...
    // 验证：各 rank 读取自己 UC 的 slot 0（因为 rank0 写的是 mc_slots + 0*N）
{
    // 只有在该 rank 真的映射了 uc_slots 时才验证（如果你的设计是“rank0 不映射 UC”，就跳过 rank0）
    bool has_uc = (uc_slots != nullptr);
    int errors = 0;

    if (has_uc) {
        std::vector<real> h(16);
        CUDA_RT_CALL(cudaMemcpy(h.data(), uc_slots /* + 0*N */, 16*sizeof(real), cudaMemcpyDeviceToHost));

        // 你初始化 d_in 时用的是 base = rank+1，但 rank0 写入的是它自己的 d_in。
        // 因此期待值应该是 base=1（rank0 的 base），再按 i%17 * 0.001 的模式。
        auto expect = [&](size_t i)->real {
            real base = real(1); // rank0 的 base
            return base + real(i % 17) * real(0.001);
        };

        for (size_t i=0; i<h.size(); ++i) {
            real e = expect(i);
            real v = h[i];
            if (fabs(double(v - e)) > 1e-6) { errors++; }
        }
    }

    int world_errors = 0;
    MPI_CALL(MPI_Allreduce(&errors, &world_errors, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD));
    if (rank == 0) {
        if (world_errors == 0) {
            printf("Validation OK: all receivers see rank0's data on UC slot 0.\n");
        } else {
            printf("Validation FAILED: %d mismatches across ranks.\n", world_errors);
        }
    }
}
    // ---------- bandwidth ----------
    const double secs = t1 - t0;
    const double bytes_per_iter_rank0 = double(N) * double(sizeof(real));  // rank0 单次写入 slot0
    const double my_bytes_per_iter    = (rank == 0) ? bytes_per_iter_rank0 : 0.0;
    ...
    // “交付总字节”（硬件复制到 world 份）仅对 rank0 有意义
    const double delivered_gb_per_rank   = (rank == 0) ? (gb_per_rank * double(world-1)) : 0.0;
    ...
    if (csv) {
        if (rank==0) {
            ...
            printf("mc_multicast_rank0_only,%d,%d,%zu,%d,%d,%s,%.6f,%.6f,%.6f\n",
        ...
        if (rank==0) {
            printf("[Rank %d | Dev %d] N=%zu | rank0 write: %.3f GB in %.3f s => %.3f GB/s\n",
            ...
            printf("World=%d | Avg per-rank BW (incl. silent ranks): %.3f GB/s | Aggregate(active ranks): %.3f GB/s | dtype=%s\n",
